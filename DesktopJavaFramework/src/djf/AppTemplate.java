...
public abstract class AppTemplate extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        ChoiceDialog<String> dialog = new ChoiceDialog<>("Language", choices);
        dialog.setTitle("Choose a language");
        dialog.setContentText("Select a language:");
        ...
        if (result.get().equals("English")) {
            ...
            try {
                ...
                single.show(props.getProperty(PROPERTIES_LOAD_ERROR_TITLE), props.getProperty(PROPERTIES_LOAD_ERROR_MESSAGE));
            }
        }
        if (result.get().equals("Spanish")) {
            try {
                // LOAD APP PROPERTIES, BOTH THE BASIC UI STUFF FOR THE FRAMEWORK
                // AND THE CUSTOM UI STUFF FOR THE WORKSPACE
